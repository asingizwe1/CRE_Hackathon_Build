...
const postNotifyLatest = (sendRequester: HTTPSendRequester): NotifyResponse => {
  const bodyBytes = new TextEncoder().encode(JSON.stringify({}))
  const body = Buffer.from(bodyBytes).toString("base64")
...
const onCronTrigger = (runtime: Runtime<Config>): string => {
  const httpClient = new HTTPClient()
  const result = httpClient
    .sendRequest(runtime, postNotifyLatest, consensusIdenticalAggregation<NotifyResponse>())()
    .result()
